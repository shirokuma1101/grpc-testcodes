...
private:
    ...
    Status GetUidPool(ServerContext* ctx, const RequestUidPoolSize* size, ResponseUidPool* uid_pool) override {
        auto ids = m_wpUidPool.lock()->GetIDs(size->size());
        uid_pool->set_start_uid(ids.first);
        uid_pool->set_end_uid(ids.second);
        return Status::OK;
    }
...
public:
    ...
    GrpcServer(const std::string& server_addr, const std::shared_ptr<UidPool>& uid_pool) {
...
int main()
{
    ...
    auto server = std::make_shared<GrpcServer>(server_addr, uid_pool);
